...
if ED == 'e':
    ...
    for i in range(int(len(input)/2)):
        if input [i*2] in codes['Alphanum_ENC_sel']:
            output += codes['Alphanum'][codes['Alphanum_ENC'].index(input[i*2+1])]
        elif input [i*2] in codes['Symbols_ENC_sel']:
            output += codes['Symbols'][codes['Symbols_ENC'].index(input[i*2+1])]
